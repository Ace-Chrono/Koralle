...
public class Enemy : MonoBehaviour
{
    ...
    private float trackingDistance = 5f;
    ...
    public void followPlayer()
    {
        ...
        {
            m_animator.speed = 1f;
            if (direction.magnitude >= trackingDistance || !hit.collider.CompareTag("Player"))
            {
                m_Rigidbody2D.velocity = Vector2.zero;
                if (!isAtInitial)
            ...
            else if (playerTransform != null && direction.magnitude <= trackingDistance && hit.collider.CompareTag("Player") && !cantDetect)
            {
                returnTimer = 0f;
                isAtInitial = false;
    ...
    private void LedgeDetect()
    {
        ...
        Debug.DrawRay(LedgeCheck.position, Vector2.down * 3f, Color.red);
